<script>
  ...
  let messageTexts = {
    ...
    Habitualidad: {
      "¿Que tanto se usa Chat-GPT?": ["¿Que tanto se usa Chat-GPT?", text1Habitualidad, {
        "Sí, lo uso cada día.": textListAnswerHabitualidad,
        "Sí, lo uso cada semana":
        textListAnswerHabitualidad,
        "Sí, lo uso raramente": textListAnswerHabitualidad,
        "No, no tenía idea": textListAnswerHabitualidad,
      }],
    },
    Lenguajes: {
      "Chat-GPT": ["¿Qué es Chat-GPT? ¿Por qué todos hablan de él?", text1Lenguaje, {
        "Lo conozco y lo uso a menudo":
          ["Lo conozco y lo uso a menudo", "No sos el único, desde que salió su popularidad fue en ascenso sin descanso."],
        "Lo conozco pero no lo uso":
          ["Lo conozco pero no lo uso", "Hay bastante gente como vos, que todavía no incorporó la herramienta pese a conocer de su existencia. Sin embargo, muchos otros ya están inmersos en el uso de la IA en su vida diaria."],
        "No lo conocía": ["No lo conocía", "Raro, te lo presento: https://chatgpt.com/"],
      }],
      "Impacto positivo/negativo": ["¿Estos modelos son buenos o malos?", text2Lenguaje, {
...
function mostrarBotones(sectionName, topic, nro_pregunta) {
  ...
  if(sectionName == "tematicas"){
    ...
    classNameOfButtonTouched = "."+sectionName+"-"+nro_pregunta+"-"+topic;
  }
  else{
...
async function nextSection(actualSectionName, nextSectionName, topic, nro_pregunta, textToShowList){
...
<main>
  ...
  <div class="container">
      ...
      <div style="display: flex;">
                ...
                >
                  {#each Object.entries(opciones[opciones.length - 1]) as [opcion, respuesta]}
                    <input
                      ...
                      value={opcion}
                      on:click={() => {
                          nextSection("btn-opciones", "btn-reinicio", tematica, nro_pregunta, respuesta);
                      }}
              ...
              <input
                ...
                on:click={() => {
                  if(preguntas.length > 1){
                    ...
                    nextSection("btn-tematicas", "btn-opciones", tematica, 0, topicsTexts[tematica]);
                  }
                }}
              />
            </div>
        ...
        <div>
          <input class="botones-opciones btn-reinicio-all" type="button" style="display: none;" value="Otro tema" on:click={
            () => {
              mostrarBotones("tematicas", "", 0);
              ocultarBotones("btn-reinicio", "all", 0);
            }
          }/>
          <input class="btn-reinicio-trabajo" type="button" style="display: none;" value="Más preguntas de Trabajo" on:click={
            () => {
              mostrarBotones("tematicas", "", 0);
              ocultarBotones("btn-reinicio", "trabajo", 0);
            }
          }/>
          <input class="btn-reinicio-habitualidad" type="button" style="display: none;" value="Más preguntas de Habitualidad" on:click={
            () => {
              mostrarBotones("tematicas", "", 0);
              ocultarBotones("btn-reinicio", "habitualidad", 0);
            }
          }/>
          <input class="btn-reinicio-lenguaje" type="button" style="display: none;" value="Más preguntas de Lenguaje" on:click={
            () => {
              mostrarBotones("tematicas", "", 0);
